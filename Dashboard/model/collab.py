...
def finder(request):
    while True:
        #choice = input('\nSearch: ')
        results=[]
        songfinder = PlaylistTracks[PlaylistTracks['Song_Artist'].str.contains(request)]['Song_Artist']
        ...
        for i in x.columns:
            ...
            list.append(dict)
        #print(list)
        ForWeb = pd.DataFrame.from_dict(list)
        ForWeb['Similarity_Value'] = ForWeb['Similarity_Value'].apply(lambda x: x[0])
        #print(ForWeb)
        ...
        radar = px.line_polar(ForWeb, r='Similarity_Value', theta='Song_Artist', line_close=True, markers= True, title=f'These songs are similar to {songfinder.iloc[0]}:')
        radar.update_traces(fill='toself')
        #radar.write_html("Dashboard\tabs\Web.html")
        #radar.show()
        return ForWeb, radar
